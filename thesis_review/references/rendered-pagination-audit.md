...
Use continuous deterministic Page IDs `P0001...` in physical-page order. `Region` distinguishes front matter, chapter, references, appendix, and back matter. `Signals` records automated or visual triage. `Inspection mode/scale` is `individual`, `small-legible-group`, or `full-scale` with the actual zoom/scale; every page needs one valid inspection record, and every suspect page needs `full-scale`. `Render artifact ID/hash` records the exact 64-hex hash of the inspected render, optionally prefixed by its PageID; do not put an arbitrary token or the source-PDF hash there. `Disposition` is `clean`, `intentional`, `finding <ID>`, or `recheck after edit`. Do not omit blank pages; explain whether each is template-mandated, chapter-structure-induced, or erroneous. Reconcile the parsed PDF page count, Stage-P inventory, CSV, and Markdown Page-ID sets; duplicate/missing/extra IDs must all be zero.
