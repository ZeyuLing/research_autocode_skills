...
Before freezing or exiting, run `python rules/scripts/validate_ai_output.py <exact-stage-ai-view-root>` until it exits `0` and its first nonempty stdout line is exactly `PASS`. This read-only scoped gate uses only the canonical AI private-view inputs and `05-ai-style-assessment.md`; it does not enumerate the finalized round or open helper, reviewer/ledger, Chair, Stage-S, old-round, or governing-local-file artifacts. The assessor may correct only its current `05-ai-style-assessment.md` in the same fresh turn. Any failure attributable to the process envelope, frozen PDF, Stage-P manifest/page inventory, governing rules, or staged validators stops the assessor and triggers Stage O's clean global retry. Validator code/stdout is mechanical rule infrastructure, not thesis evidence or a source of style findings.
...
After prose edits produce a newly frozen PDF, Stage O launches a fresh assessor without showing it the previous assessment. Production runner v1 performs no Stage-V comparison and accepts no prior AI report. A future separately implemented longitudinal extension could compare specifically identified current and prior AI reports only after the complete current round is frozen; it must never expose the prior report to the current chair or alter current grades, findings, revision ledger, or clean summary.
