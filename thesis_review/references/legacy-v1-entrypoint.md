# Legacy v1 entrypoint — not the active review protocol

Retained for diagnosing existing v1 artifacts. New reviews use review-v2.md.
...
Concurrent actors may contend only while performing the pure read-only process-seal verification before or after launch. That verifier alone may wait for the workspace kernel lock for at most 30 seconds with short retries: Windows retries only sharing/lock violations, Linux retries only `BlockingIOError`, and every other acquisition error fails immediately. After acquiring the lock, the verifier reruns the complete seal validation from the beginning. Initialize, seal, quarantine, cleanup, and every other mutating operation retain fail-fast nonblocking locking and must never wait and then write. The workspace lock is released when each seal verification ends and is never held across an actor's execution lifetime.
