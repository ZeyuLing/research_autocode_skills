...
After every R/AI target has passed its own scoped gate and frozen, but before the
Chair is launched, start one different fresh `SA-<target>` actor for each target.
