...
The validator derives the canonical local allowlists, rather than trusting prose. P opens the process envelope, `SKILL.md`, the ten required reference files in their documented order, `rules/scripts/validate_review_bundle.py`, `rules/scripts/validate_stage_p_output.py`, process-bound governing files, and the frozen PDF; P has no helper inputs and may not probe `helpers/`. Each R actor opens the ordinary rule/PDF prefix plus the seven current packet/policy/inventory artifacts and no peer output; only doctoral R5 has its separately documented validator-script insertion. AI opens only the process envelope, `SKILL.md`, clean-room/report/AI rules, frozen PDF, manifest, and page inventory. C opens the complete current rule/PDF/packet prefix followed by `02`--`04`, every current R report, and the current AI report. S opens only the exact current summary-source sequence. A missing, extra, duplicated, reordered, or substring-only basename invalidates the stage. Public endpoints are a duplicate-free subset of that actor's current policy/citation authority; AI and S use `[none]`.
