...
`<closed-exec-items-in-any-order>` contains exactly once each of `--json`, `--ephemeral`, `--ignore-user-config`, and `--ignore-rules`; exactly one `-C <actor-workspace>` pair; exactly one `--sandbox workspace-write` pair; exactly one of `--disable multi_agent`, `--disable=multi_agent`, or `-c features.multi_agent=false`; and, optionally, at most once `--skip-git-repo-check`. No sandbox/approval bypass is admissible. If present, the sole `--search` occurs before `exec`. The sole stdin marker `-` is last. No other flag, configuration override, positional argument, subcommand, model/profile/image/add-directory option, or second `exec` is permitted.
