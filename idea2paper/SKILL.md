...
Store every numbered figure or table as one complete LaTeX unit in its own file.
Put figure floats, source-anchored `\captionof{figure}` blocks, and the non-floating
title teaser under `paper/figures/*.tex`; put table floats and source-anchored
`\captionof{table}` blocks under `paper/tables/*.tex`. Each dedicated file must
contain exactly one artifact, including its placement wrapper, `\includegraphics`
or `tabular`, caption, label, and artifact-local TODOs. Section and appendix files
must contain only `\input{figures/<name>}` or `\input{tables/<name>}` at the
artifact's intended source position; never paste a figure/table environment or
`\captionof` unit into prose files, and never combine several artifacts in one
dedicated file. `sections/teaser.tex` is a one-line indirection containing only
`\input{figures/fig_teaser}`. Raster assets remain alongside these LaTeX wrappers
under `paper/figures/`; a `.png` and its `.tex` wrapper are distinct artifacts.
