...
When migrating into the official template, place the optional Limitations input before the sole Conclusion input, then put `idea2paper:end-body` immediately after Conclusion. This makes Conclusion the final body section and prevents unresolved body floats from being normalized as post-conclusion material. If a venue-exempt AI disclosure follows, put `idea2paper:end-exempt` immediately after that one input; otherwise place the two labels together. Put `idea2paper:end-references` immediately after the bibliography and begin `appendix/appendix.tex` with `\appendix`. The compile gate binds this canonical order to the real section/bibliography/appendix commands, caps the exempt disclosure to one additional page, and uses the appropriate body label according to whether references count.
