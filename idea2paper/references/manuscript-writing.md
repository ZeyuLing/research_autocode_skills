...
Treat file organization as part of the artifact contract. Every numbered figure,
table, non-floating teaser, or source-anchored `\captionof` unit belongs in exactly
one dedicated `.tex` file: figures under `paper/figures/` and tables under
`paper/tables/`. The dedicated file owns the complete placement wrapper, graphical
or tabular body, caption, label, spacing, and artifact-local TODOs, and contains no
section headings or unrelated prose. A section or appendix preserves argumentative
order with a single `\input{figures/<name>}` or `\input{tables/<name>}` where the
artifact used to appear. Do not group multiple artifacts into one file. Keep
`sections/teaser.tex` as the single line `\input{figures/fig_teaser}` so the
front-matter contract remains explicit while the teaser itself follows the same
one-artifact-per-file rule. Image files such as `.png` stay under
`paper/figures/`; they do not replace the dedicated LaTeX wrapper.
