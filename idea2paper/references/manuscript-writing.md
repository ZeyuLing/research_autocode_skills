...
When the venue permits a teaser, its rendered order is strictly **Title -> Teaser
-> Authors -> Abstract**. Never place it above the title, after the author block,
or as an ordinary float. Keep the official template file byte-for-byte unchanged:
use `\IdeaTwoPaperPatchTitleTeaser{<exact title-token anchor>}{\input{sections/teaser}}`
before `\begin{document}` to splice the non-floating
`IdeaTwoPaperTitleTeaser` environment into `\@maketitle`. Compilation must fail
when the template anchor no longer matches. If venue instructions forbid
title-block insertion, omit the teaser and record that decision; do not silently
