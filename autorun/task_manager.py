...
def cmd_peek(_args):
    """Peek at next pending task WITHOUT marking it in_progress. For dry-run / preview."""
    conn = get_connection()
    row = conn.execute(
        "SELECT * FROM tasks WHERE status='pending' ORDER BY position ASC LIMIT 1"
    ).fetchone()
...
def cmd_get(args):
    """Get a task by ID without changing its status."""
    conn = get_connection()
...
def cmd_start(args):
    """Mark a specific task as in_progress by ID."""
    conn = get_connection()
    row = conn.execute("SELECT * FROM tasks WHERE id=?", (args.id,)).fetchone()
    if row is None:
        print(json.dumps({"ok": False, "error": f"Task {args.id} not found"}))
        ...
        return
    started = now_str()
    conn.execute("UPDATE tasks SET status='in_progress', started_at=? WHERE id=?", (started, args.id))
    conn.commit()
    total = conn.execute("SELECT COUNT(*) as c FROM tasks WHERE status != 'decomposed'").fetchone()["c"]
    completed = conn.execute("SELECT COUNT(*) as c FROM tasks WHERE status='completed'").fetchone()["c"]
    print(json.dumps({"ok": True, "id": args.id, "started_at": started, "progress": f"{completed + 1}/{total}"}))
    conn.close()


def cmd_list_pending(_args):
    """List all pending tasks for Team Lead analysis."""
    conn = get_connection()
    rows = conn.execute(
        "SELECT id, description, tag, parent_id FROM tasks WHERE status='pending' ORDER BY position ASC"
    ).fetchall()
    total = conn.execute("SELECT COUNT(*) as c FROM tasks WHERE status != 'decomposed'").fetchone()["c"]
    completed = conn.execute("SELECT COUNT(*) as c FROM tasks WHERE status='completed'").fetchone()["c"]
    tasks = [{"id": r["id"], "description": r["description"], "tag": r["tag"], "parent_id": r["parent_id"]} for r in rows]
    conn.close()
    print(json.dumps({"pending": len(tasks), "total": total, "completed": completed, "tasks": tasks}, ensure_ascii=False))
...
def cmd_list(_args):
    """List tasks summary: counts by status + pending task list (id & description only)."""
    conn = get_connection()
    ...
    # Only fetch pending & in_progress tasks (brief info) — these are actionable
    pending = conn.execute(
        "SELECT id, description, tag FROM tasks WHERE status IN ('pending','in_progress') ORDER BY position ASC"
    ).fetchall()
    conn.close()

    pending_list = [{"id": r["id"], "description": r["description"], "tag": r["tag"]} for r in pending]
...
def main():
    ...
    # peek
    sub.add_parser("peek", help="Preview next pending task without changing status")

    # get
    p_get = sub.add_parser("get", help="Get a task by ID without changing its status")
    p_get.add_argument("id", type=int)

    # start
    p_start = sub.add_parser("start", help="Mark a specific task as in_progress by ID")
    p_start.add_argument("id", type=int)

    # list-pending
    sub.add_parser("list-pending", help="List all pending tasks for Team Lead analysis")
    ...
    # list
    sub.add_parser("list", help="List task summary (counts + pending tasks)")
